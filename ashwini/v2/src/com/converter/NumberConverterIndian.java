...
import static com.converter.StringConstants.LAC;
import static com.converter.StringConstants.CRORE;
import static com.converter.StringConstants.NO_RESULT;

class NumberConverterIndian {
	...
	private static String getSingleDigitNumber(int number) {
		return NumberConstants.numberMap.get(number);
	}

	private static String getTwoDigitNumber(int number) {
		if (number > 20 && number % 10 != 0) {
			return NumberConstants.numberMap.get(number - number % 10) + SPACE
					+ NumberConstants.numberMap.get(number - (number / 10) * 10);
		} else {
			return NumberConstants.numberMap.get(number);
		}
	}
	...
	private static String getThreeDigitNumber(int number) {
		if ((number % 100) == 0) {
			return NumberConstants.numberMap.get(number / 100) + SPACE + HUNDRED;
		} else {
	...
	private static String getFourDigitNumber(int number) {
		if ((number % 1000) == 0) {
			return NumberConstants.numberMap.get(number / 1000) + SPACE + THOUSAND;
		} else {
	...
	private static String getSixDigitNumber(int number) {
		if ((number % 100000) == 0) {
			return NumberConstants.numberMap.get(number / 100000) + SPACE + LAC;
		} else {
	...
	private static String getEightDigitNumber(int number) {
		if ((number % 10000000) == 0) {
			return NumberConstants.numberMap.get(number / 10000000) + SPACE + CRORE;
		} else {
